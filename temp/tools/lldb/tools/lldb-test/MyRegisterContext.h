  ...
  MyRegisterContext(
      ...
      std::function<void(const std::string &, std::bitset<128>)>
          setRegisterValueFunc,
                    const ArchSpec &target_arch, Thread &thread,
...
private:
  std::function<std::bitset<128>(const std::string &)> m_getRegisterValueFunc;
  std::function<void(const std::string &, std::bitset<128>)> m_setRegisterValueFunc;
  ArchSpec m_arch;
