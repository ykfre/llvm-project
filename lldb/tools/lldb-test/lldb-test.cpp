...
public:
  Process2(lldb::TargetSP target_sp, lldb::ListenerSP listener_sp, const RunArgs& RunArgs)
...
public:
  Thread2(RunArgs RunArgs, lldb_private::Process &process, lldb::tid_t tid,
      bool use_invalid_index_id = false):Thread(process,tid,use_invalid_index_id), m_RunArgs(RunArgs) {
  }
    ...
    if (!m_register_context) {
      m_register_context =
          lldb::RegisterContextSP(new lldb_private::MyRegisterContext(
              m_RunArgs.getRegisterValue, m_RunArgs.setRegisterValue, CalculateTarget()->GetArchitecture(),
              *this, 0));
    }
...
public:
  ...
  lldb::ProcessSP CalculateProcess() override {
    if (!m_process.get()) {
      auto listener = lldb_private::Listener::MakeListener("listen");
      m_process = lldb::ProcessSP(new Process2(CalculateTarget(), listener, m_RunArgs));
  ...
  lldb::ThreadSP CalculateThread() override {
    if (!m_thread.get()) {
      ...
      m_thread = lldb::ThreadSP(
          new Thread2(m_RunArgs,*CalculateProcess(),
                      m_RunArgs.getThreadId()));
    }
    return m_thread;
  };
...
bool run(RunArgs runArgs) {
  ...
  assert(targetList
             .CreateTarget(*debugger, runArgs.file_path, "", load_dependent_files,
                           nullptr,
                           target_sp)
             .Success());
  lldb_private::FileSpec fileSpec(runArgs.file_path);
  ...
  if (runArgs.bpAddress) {
    ...
    runArgs.addBreakpoint(
        (void*)(entryPointAddress.get().GetLoadAddress(exe_ctx.CalculateTarget().get())));
  }
