...
public:
  ...
  void Suspend() { SuspendThread(g_thread.native_handle()); }
  ...
  size_t DoWriteMemory(lldb::addr_t vm_addr, const void *buf, size_t size,
                       lldb_private::Status &error) override {
    memcpy((void *)vm_addr, (void *)buf, size);
    return size;
  }
...
class Thread2 : public lldb_private::Thread {
  using lldb_private::Thread::Thread;
  ...
  bool RestoreRegisterStateFromCheckpoint(ThreadStateCheckpoint &saved_state) {
    SuspendThread(g_thread.native_handle());
    bool result =
        lldb_private::Thread::RestoreRegisterStateFromCheckpoint(saved_state);
    ResumeThread(g_thread.native_handle());
    return result;
  }
    ...
    if (!m_register_context) {
      m_register_context =
          lldb::RegisterContextSP(new lldb_private::MyRegisterContext(
              g_thread.native_handle(), CalculateTarget()->GetArchitecture(),
              *this, 0));
    }
...
public:
  MyCtx(const lldb::TargetSP target) { m_target = target; }
  ...
  lldb::ProcessSP CalculateProcess() override {
    if (!m_process.get()) {
      auto listener = lldb_private::Listener::MakeListener("listen");
      m_process = lldb::ProcessSP(new Process2(CalculateTarget(), listener));
    }
    return m_process;
  }
  ...
  lldb::ThreadSP CalculateThread() override {
    if (!m_thread.get()) {
      ...
      std::cout << GetThreadId(g_thread.native_handle());
      m_thread = lldb::ThreadSP(
          new Thread2(*CalculateProcess(),
                      (lldb::tid_t)GetThreadId(g_thread.native_handle())));
    }
    return m_thread;
  };
  ...
  if (result_valobj_sp) {
    ...
    if (result_valobj_sp->GetError().Success()) {
      ...
      options.SetMaximumPointerDepth(
          {lldb_private::DumpValueObjectOptions::PointerDepth::Mode::Always, 5});
...
int main() {
  auto module = LoadLibraryA(
      "C:\\Users\\idowe\\source\\repos\\Project1\\Project1\\a.exe");
  ...
  lldb_private::Platform::SetHostPlatform(
      lldb::PlatformSP(new Platform2(true)));
  ...
  assert(debugger->EnableLog("lldb", {"expr", "unwind"}, "me.txt", 0,
                             *log_stream_sp));
  ...
  MyCtx scope(target_sp);
  lldb::ValueObjectSP value;
  auto exe_ctx = new MyCtx(target_sp);
