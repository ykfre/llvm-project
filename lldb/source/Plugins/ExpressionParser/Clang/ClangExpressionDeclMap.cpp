...
void ClangExpressionDeclMap::LookupInModulesDeclVendor(
  ...
  if (!modules_decl_vendor->FindDecls((clang::DeclContext *)context.m_decl_context, name,
  ...
  for (const auto& decl: decls) {
    LLDB_LOG(log,
             ...
             "\"{1}\" in the modules",
             current_id, name);
    auto copied_decl = CopyDecl(decl);
    if (!copied_decl) {
      LLDB_LOG(log,
    ...
    } else if (auto copied_var = dyn_cast<clang::CXXRecordDecl>(copied_decl)) {
      context.AddNamedDecl(copied_var);
      context.m_found.type = true;
    } else if (auto copied_var = dyn_cast<clang::TemplateDecl>(copied_decl)) {
      context.AddNamedDecl(copied_var);
      context.m_found.type = true;
    } else if (auto copied_var = dyn_cast<clang::ClassTemplateSpecializationDecl>(copied_decl)) {
      context.AddNamedDecl(copied_var);
      context.m_found.type = true;
    }
    else if (auto copied_var = dyn_cast<clang::ClassTemplatePartialSpecializationDecl>(copied_decl)) {
      context.AddNamedDecl(copied_var);
      context.m_found.type = true;
    }
  }
...
void ClangExpressionDeclMap::LookupFunction(NameSearchContext &context,
  ...
  if (target) {
    if (ClangModulesDeclVendor *decl_vendor =
      ...
      decl_vendor->FindDecls((clang::DeclContext* )context.m_decl_context, name, false, UINT32_MAX, decls_from_modules);
    }
  }
